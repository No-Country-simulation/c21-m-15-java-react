...
const GestionOnline = () => {
  ...
  // Cargar los horarios disponibles para el médico seleccionado y la fecha seleccionada
  useEffect(() => {
    const fetchHorariosDisponibles = async () => {
      if (!medicoSeleccionado || !selectedDate) return;

      try {
        const formattedDate = format(selectedDate, "yyyy-MM-dd"); // Formatear la fecha seleccionada
        const response = await axios.get(`http://localhost:8080/api/horarios/${medicoSeleccionado.id}?fecha=${formattedDate}`);
  ...
  return (
    <Container sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, width: "100%", minHeight: "100vh" }}>
      ...
      <Box sx={{ flex: 2, padding: 2, boxShadow: 3 }}>
        ...
        {errorHorarios ? (
          <Typography variant="body2" color="error">{errorHorarios}</Typography>
        ) : (
          <Box>
            <Typography variant="body2"><strong>Horarios Disponibles:</strong></Typography>
            <ul>
              {horariosDisponibles.length > 0 ? (
                ...
                <Typography variant="body2">No hay horarios disponibles para esta fecha.</Typography>
              )}
            </ul>
          </Box>
        )}
